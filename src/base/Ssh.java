...
public class Ssh {
	...
	private Expect4j getExpect() {
		try {
			System.out.println(String.format("Start logging to %s@%s:%s", senv.getUser(), senv.getIp(), senv.getPort()));
			JSch jsch = new JSch();
			...
			channel.connect();
			System.out.println(String.format("Logging to %s@%s:%s successfully!", senv.getUser(), senv.getIp(), senv.getPort()));
			return expect;
		} catch (Exception ex) {
			System.out.println("Connect to " + senv.getIp() + ":" + senv.getPort() + " failed,please check your username and password!");
			ex.printStackTrace();
		}
		return null;
	}
